...
| `fixed-in-<push-signout>` | Same placeholder convention as `fixed-in-<wave4b>`, for the sign-out push release fix: the writer of the row is composing the commit and cannot know its sha. Resolve with `git log --oneline --grep 'push registration'`. |
...
## Found by the Wave-5 push proof — 2026-07-30

One finding, raised while working RUNBOOK §9.4's manual pass on live hardware and
recorded there as an open defect at the time, because `src/store/push.ts`
belonged to another worker. It is the first finding to use the slug scheme this
document has recommended since Wave 4; there is no `S`/`C`/`M`/`P` label for it
and there should not be one.

| Id | Severity | Disposition |
| --- | --- | --- |
| **`PUSH-SIGNOUT`** | major | `fixed-in-<push-signout>` — **sign-out gave back the browser subscription but not the row.** `resetPush()` cleared the store and called `unsubscribeThisDevice()`, which is browser-side only; nothing deleted the matching `public.push_subscriptions` row, so a signed-out user's endpoint and both subscription keys stayed in the table. Measured live: subscribe through the product UI, confirm one row, sign out through Settings — the browser subscription became null and `select count(*)` still answered **1**. **Severity is about the offline case, not the ordinary one.** Ordinarily the unsubscribe lands first, the endpoint is dead at FCM, the next drain gets a `410` and prunes the row, and the cost is stale retention. But `unsubscribeThisDevice()` swallows its own failure, so an **offline** sign-out left the row *and* a live registration, and the next person on that machine received the previous user's notifications — the one thing in this app that arrives with the app closed. **The prescribed one-line fix would not have worked, and would have looked like it did.** A `delete … where endpoint = …` inside `resetPush()` is issued from App.tsx's sign-out teardown, which is reached only *after* `session` has gone null — and `push_subscriptions` is owner-only RLS (`0011`), so that delete matches no rows and **returns no error**. So the fix is sequencing as much as deletion: `store/auth.signOut()` awaits a new `releasePushForSignOut()` **before** `supabase.auth.signOut()`, while the token is still on the request. The endpoint is read **up front** — from the store, or from `currentDeviceSubscription()` when Settings was never opened this session, which is the ordinary sign-out — so the delete survives an unsubscribe that throws, which is exactly the failure the fix exists for. The call is bounded at 4 s so a captive portal cannot hang sign-out on a socket timeout, and `resetPush()` keeps a best-effort release as the backstop for the sign-outs that never reach `signOut()`: an expired session, a revoked token, a sign-out in another tab. Costs the app its **first store-to-store import** (`store/auth` → `store/push`), which is noted at the import and is acyclic. Pinned by `src/store/push.test.ts`, which asserts the **order** rather than just the delete — for the RLS reason above, "a delete was issued" would pass against a fix that deletes nobody's row. **Verified live 2026-07-31 against `c373b5f`: 0 → 1 → 0**, one device through the product UI — and the shipped chunk was read back to confirm the *ordering* survived minification, not merely that a delete exists in the bundle. [`EVIDENCE/push-signout-verification.md`](EVIDENCE/push-signout-verification.md) records both halves, who measured which, and the four things the run does not establish (the offline path, iOS, multi-device, and that the two counts are owner-reported rather than tool-captured). |

**Still true after the fix, and both are in RUNBOOK §9.4 step 9.** An offline
sign-out cannot delete the row, because nothing can reach PostgREST — a shared
machine should be signed out online. And a row that survives anyway is removed
with `delete from public.push_subscriptions where user_id = '<uuid>';`.
