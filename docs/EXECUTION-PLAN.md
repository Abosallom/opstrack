...
7. **CSS prefix registry.** Each co-located sheet owns exactly one prefix and may not style another's, nor a `global.css` primitive:
   ...
   **RETIRED BY THE MAP COLLAPSE** (struck through above; the sheets are deleted, the prefixes are free and must not be re-used for anything else without a fresh entry here): `.cap-*` → superseded by `map-capture.css → .mcap-*` · `.fu-*` → superseded by `map-list.css → .mtree-list-*` · `.bd-*` → superseded by `map-board.css → .mbd-*` · `.tl-*` and `pages/tracks/tree.css → .tree-*` → superseded by `map-branch.css → .mbr-*` · `.db-*` → superseded by `map-numbers.css → .mnum-*`. The one rule outside those sheets that still named a retired prefix — `global.css`'s `--vocab-ink` recipe, which listed `.bd-col-title .bd-rail .bd-overflow-label` — was repointed at `.mbd-*` in the same commit; left alone it would have silently stopped applying.
   ...
   **Added by the password-recovery pass** (one sheet): `reset.css → .rst-*` — the `/reset` screen: asking for a recovery link, and setting the password one arrived with. Registered rather than folded into `signin.css` or `claim.css` because it is a THIRD shell-less screen with its own full-viewport frame, and either of those would have had to grow rules for a screen it does not own — `signin.css` in particular already carries a "check your email" panel and a one-time-code disclosure that have nothing to do with this route.

   **Added by the map decomposition** (`src/components/map/`, continuing the disjoint-slice exception the interactive Mindtree established above — the prefix still has exactly one owner, the feature): `map-list.css → .mtree-list-*` — the list rendering of the same tree. **AMENDED at the integration:** the split wrapper it also shipped, `.mtree-list-split`, is GONE, superseded by `map-panel.css`'s `.mpan-split` — see that entry below. · `map-capture.css → .mcap-*` — the composer that lives on the canvas: the input row, the token chips, the live problems panel, the ambiguity picker, the suggested-tag row and the token cheatsheet, plus the phone dock that fixes the bar to the block end and inverts its visual order. **This line WAS the `.mcap-*` RESERVED placeholder.** The sheet now exists, `MapCapture.test.tsx` imports it (a missing file fails that suite at transform time) and pins the exact `.mcap-` name set, and its `--mcap-dock-inset` was retuned from `calc(53px + safe-area)` to the bare safe-area inset when the tab bar it cleared was deleted. · `map-lens.css → .mlens-*` — the five lens chips and the map⇄ledger switch. A prefix of its own rather than a slice of `.mtree-`: this row is the SHELL's chrome, it decides which surface the shell shows, and it outlives any one of them. · `map-panel.css → .mpan-*` — the dock: the `.mpan-split` wrapper that keeps the panel a SIBLING of `.mtree-canvas` (whose `touch-action: none` intersects DOWN the ancestor chain, so a list inside it cannot be finger-scrolled at all), the inline-end rail at ≥768px, and the non-modal three-detent bottom sheet below it. **It SUPERSEDES `.mtree-list-split`** — map-list.css shipped that wrapper when the list was the only thing that could sit beside the canvas; the shell now puts five panel subjects there over four stages, so map-list.css keeps `.mtree-list-*` for the list itself and its `-split` rule is gone. · `map-branch.css → .mbr-*` — the branch panel: what a node IS, then a track's open work, then its history. **`MapBranch.tsx`, `MapBranchDetail.tsx` and `MapBranchHistory.tsx` are one component split for size and share this one sheet** — three files, one prefix, no new entry, on the §1.0.4 rule the Mindtree cites. `MapBranchDetail.test.tsx` gates it: every `.mbr-*` name that band renders must have a rule in the sheet, `.mbr-detail` excepted as an identity. It replaces `pages/tracks/tree.css → .tree-*` and `pages/tracks/timeline.css → .tl-*`, both deleted with their pages. · `map-changes.css → .mchg-*` — the day-grouped activity record inside the map's panel. It replaces the `/notifications` page; `components/notifications.css` still owns the ROW, which the bell's popover and this panel share. · `map-board.css → .mbd-*` — the board stage: the column scroller, the columns and their collapsed rails, the quick-add composer, the drop slot and the overflow rail. Supersedes `board.css → .bd-*`, retired with `pages/Board.tsx`. · `map-numbers.css → .mnum-*` — the numbers stage and the six tiles in the panel beside it, one prefix over two components because they are one surface answering one question. It replaces `pages/dashboard.css` (`.db-*`), deleted with the `/dashboard` route. Its breakpoint is an `@container` query on `.mnum-grid`, not a viewport media query, because this stage shares its window with the panel. · `map-mode.css → .mmode-*` — the two mode entrances in the shell header (`components/map/MapModeBar.tsx`) and the frame every mode route renders inside (`components/map/ModeFrame.tsx`). One sheet for two components on the §1.0.4 rule the Mindtree cites: they are the two halves of one idea — leaving the map and coming back — and two sheets would have to agree about the same trail metrics forever.
   ...
   **Added by Wave B** (0023–0025; three sheets, one per screen, none folded into a neighbour): `pages/settings/structure.css → .str-*` — Settings › Structure: the tree beneath every track (`map_nodes`, 0023). Kept out of `groups.css` and `admin.css` for the reason `groups.css` was kept out of `admin.css`: those two style flat lists inside a container, this one styles a tree six levels deep, and it owns the one invariant neither has — an indent bound to a custom property (`padding-inline-start: calc(var(--depth) * 1.25rem)`) plus a `border-inline-start` guide rule that must mirror with it, or the whole hierarchy renders flush against one edge in Arabic · `pages/settings/catalogue.css → .cat-*` — Settings › Catalogue: the two unfrozen lists (`use_cases`, `map_node_kinds`) with their add form, per-row editor, hide toggle and the delete-refusal sentence. Kept out of `vocab.css` on the same rule: the two screens look alike and are not — Vocabulary spends its layout on a colour picker, a resolution preview and two day counts, none of which exist here, and this one spends its layout on an add form, a "no Arabic name yet" state and a refusal that has to name a count · `pages/settings/roles.css → .rol-*` — Settings › Roles & permissions: the standing note about what is and is not configurable, the role card's head, its rename panel, the permission switch list with its two lines of prose per row, and the create form. Kept out of `groups.css` and `admin.css` on the rule that split those two: it is the part of the settings tree writing a THIRD pair of tables (`roles`, `role_permissions`), and it owns the one layout neither has — a switch whose explanation is part of the control.

   **Added by the continuous dive** (`feat/map-hierarchy`; two sheets, one new prefix pair and one slice of `.mtree-`): `src/components/map/map-altitude.css → .mdive-*` and `→ .malt-*` — ONE sheet, two prefixes, and the filename is now wrong on purpose. It was written for `MapAltitude`, the four-stop ladder, which is deleted; what stands there is `.mdive-*`, the CONTINUOUS dive rail that replaced eleven controls (three zoom buttons, the four named stops, Fit, Expand all, Collapse all), plus `.malt-*`, the group-by disclosure `MapToolbar` still renders from the same sheet. Renaming the file is two import lines and a registry line for zero behaviour, so it is recorded here rather than done inside a change that has to be reviewable against a contract. · `src/components/mindtree/mind-ring.css → .mring-*` — a disjoint slice of the mindtree feature's own prefix, on the exception §1.0.7 already grants above: the ring guides and `MindWorldRim`'s boundary, name and match arcs — the marks that exist only while a world is OPENING or IS the frame, and that `pages/mindtree.css` therefore has no rule for at any other distance.

   **Added by the flat tidy tree** (`feat/map-hierarchy`; NO new sheet and NO new prefix, recorded here because the registry enumerates FAMILIES and a reader looking for `.mtree-block*` must find it): `src/pages/mindtree.css` gains `.mtree-block*` — the BLOCK CONTAINERS that replace the per-child connectors on the vertical wrapped tidy tree: one rounded rectangle per parent enclosing its whole subtree, painted behind the cards, joined to the parent by a single stub. Drawn by `src/components/mindtree/MindBlock.tsx`, which like `MindNode.tsx` and `MindEdge.tsx` imports no stylesheet of its own; geometry in `src/lib/mindtree/blocks.ts`. It sits in the same sheet as `.mtree-edge*` deliberately rather than in a sibling file: the two are alternative answers to one question ("who does this card belong to"), only one is on screen at a time, and splitting them would put a rule that exists BECAUSE of the other in a file that cannot mention it. The sheet's own header carries the contrast re-derivation the fill forced — the empty organization's `--text-faint` label is what capped it at 5%.

   **Added by the Jira reader** (one sheet): `pages/settings/jira.css → .jir-*` — Settings › Jira: the standing read-only declaration, the field/status mapping lists, the reconciliation breakdown and the results table with its own horizontal scroller. Kept out of `structure.css` and `catalogue.css` on the rule that split those two: it is the only screen in the settings tree rendering a wide table of rows that came from OUTSIDE this workspace, and the only one whose accent colour is a promise rather than a state.

   **Added by the Portfolio lens** (`feat/map-hierarchy`, wave 4; one sheet, one prefix): `src/components/map/portfolio.css → .pf-*` — the portfolio STAGE: the `?by=` chip row and the `?risk=` toggle above it, the nine-column organization table and its seven-column roll-up, the inline stage `<select>` on every row, the bulk bar behind a selection, and the three sentences that name an unconfigured state (no ladder, no threshold, a truncated read). Kept OUT of `mindtree.css`'s `.mtree-tbl*` slice, on the rule that split `map-board.css` from it: this is a stage that REPLACES the canvas and owns chrome, an editable cell and a bulk bar, while `.mtree-tbl*` is a rendering of the same tree the canvas draws — two surfaces with two lifetimes, and `mindtree.css` would otherwise grow rules for a `<select>` inside a table cell that it has no other reason to know about. It deliberately does NOT reuse `map-lens.css`'s `.mlens-chip`: that class is the shell rail's, it is width-gated on the phone rail's own breakpoint, and inheriting it would move this row every time the lens rail was retuned. `PortfolioStage.test.tsx` gates the names it renders; `.pf-risk-word`, `.pf-orgname` and `.pf-flag` each carry a rule.
