...
// has gone red. One glance, no rows read.
//
// THIS FILE COMPOSES; IT DOES NOT COMPUTE, and after the decomposition it does
// not even wire. Eleven pure modules own the hard parts (lib/mindtree/*), eight
// hooks own the wiring (pages/map/*), and three components own the chrome
...
//   useMapUrl        ?focus= and ?group=, called late so its two effects keep
//                    the position they had in the undivided file
//
...
import { EMPTY_FILTER, isFilterEmpty } from '../lib/entryFilter'
...
export default function Mindtree(): ReactElement {
  ...
  const { pulses, motion } = useMindPulses({
    tree: model.view === 'map' ? focus.drawnRoot : null,
    paused: model.dragging,
    enabled: model.view === 'map',
  })
  ...
  return (
    <div className="mtree">
      ...
      {model.view === 'map' && (
        <Breadcrumb trail={focus.focusView.trail} onFocus={focus.focusBranch} />
      )}
      ...
      {model.view === 'map' && model.selectionCount > 0 && (
      ...
      {model.view === 'map' && <MindDragLayer controller={dragController} />}
